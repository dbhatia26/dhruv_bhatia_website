...
- [ ] **Merge `main` into `splitter`** to bring the Next 16 upgrade into this branch before resuming Phase 3. Expect a real conflict in `package.json`/`package-lock.json` (Next 16/React 19 on `main`'s side vs. drizzle/vitest/pglite additions already on `splitter`). Resolve `package.json` by hand, then delete and regenerate `package-lock.json` with `npm install` rather than hand-merging the lockfile.
