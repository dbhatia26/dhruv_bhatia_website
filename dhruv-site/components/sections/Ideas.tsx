  ...
  return (
    <article
      ...
      style={{
        ...
        padding: '24px', cursor: 'pointer', transition: 'all 0.35s ease',
        display: 'flex', flexDirection: 'column', minHeight: '220px',
        ...(expanded ? { background: 'var(--card-hover-bg)', borderColor: 'rgba(0,212,255,0.22)', transform: 'translateY(-3px)', boxShadow: '0 12px 32px rgba(0,0,0,0.15)' } : {}),
      }}
    >
      ...
      <div style={{ flex: 1, overflow: expanded ? 'visible' : 'hidden', display: expanded ? 'block' : '-webkit-box', WebkitLineClamp: expanded ? 'unset' : 3, WebkitBoxOrient: 'vertical' as const }}>
        {idea.excerpts.map((p, i) => (
          <p key={i} style={{ fontSize: '12.5px', color: 'var(--text-muted)', lineHeight: 1.65, marginBottom: i < idea.excerpts.length - 1 ? '8px' : '0' }}>{p}</p>
        ))}
      </div>
      {!expanded && (
        <p style={{ fontFamily: 'DM Mono, monospace', fontSize: '10px', letterSpacing: '0.12em', textTransform: 'uppercase', color: 'rgba(0,212,255,0.5)', marginTop: '10px', flexShrink: 0 }}>
          Hover to read more ↓
        </p>
      )}
...
export default function Ideas() {
  ...
  return (
    <section id="ideas" ref={ref} style={{ padding: '120px 24px', borderTop: '1px solid var(--border)' }}>
      <div style={{ maxWidth: '1000px', margin: '0 auto' }}>
        ...
        <h2 className="fade-section" style={{ fontFamily: 'Syne, sans-serif', fontSize: 'clamp(2rem,4vw,3rem)', fontWeight: 700, color: 'var(--text-strong)', marginBottom: '10px' }}>Thinking Out Loud</h2>
        <p className="fade-section" style={{ color: 'var(--text-muted)', fontSize: '1.05rem', fontWeight: 300, marginBottom: '44px' }}>Short thoughts on the future of analytics, AI, and decision systems.</p>
        <div className="fade-section" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '14px', alignItems: 'start' }}>
