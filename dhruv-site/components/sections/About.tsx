...
export default function About() {
  ...
  return (
    <section id="about" ref={ref} style={{ padding: 'clamp(60px,10vw,120px) clamp(16px,5vw,24px)', position: 'relative' }}>
      <style>{`
        ...
        @media (max-width: 900px) and (min-width: 701px) { .pillars-grid { grid-template-columns: repeat(2, 1fr); } }
      `}</style>
      <div style={{ maxWidth: '1000px', margin: '0 auto' }}>
        ...
        <h2 className="fade-section" style={{ fontFamily: 'Syne, sans-serif', fontSize: 'clamp(1.8rem,5vw,3rem)', fontWeight: 700, color: 'var(--text-strong)', lineHeight: 1.2, maxWidth: '700px', marginBottom: '24px' }}>
          Analytics should move from <span className="gradient-text-static">dashboards</span> to <span className="gradient-text-static">conversational intelligence.</span>
        </h2>
        <div className="fade-section" style={{ fontSize: 'clamp(0.95rem,2vw,1.1rem)', color: 'var(--text-muted)', lineHeight: 1.75, fontWeight: 300, maxWidth: '600px', marginBottom: '48px', display: 'flex', flexDirection: 'column', gap: '14px' }}>
        ...
        <div className="fade-section pillars-grid" style={{ marginBottom: '56px' }}>
          {pillars.map((p) => (
            <div key={p.title} className="card-glass" style={{ borderRadius: '12px', padding: 'clamp(16px,3vw,24px)' }}>
        ...
        <div className="fade-section">
          ...
          {timeline.map((item, i) => (
            <div key={i} style={{ display: 'flex', gap: '16px', paddingBottom: i < timeline.length - 1 ? '24px' : '0' }}>
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', paddingTop: '4px', flexShrink: 0 }}>
                <div style={{ width: '10px', height: '10px', borderRadius: '50%', border: item.current ? '1px solid var(--cyan)' : '1px solid var(--border)', background: item.current ? 'rgba(0,212,255,0.18)' : 'var(--surface)', boxShadow: item.current ? '0 0 8px rgba(0,212,255,0.5)' : 'none' }} />
              ...
              <div style={{ paddingBottom: '4px' }}>
                <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'baseline', gap: '6px', marginBottom: '4px' }}>
                  <span style={{ fontFamily: 'Syne, sans-serif', fontSize: 'clamp(12px,2vw,13px)', fontWeight: 600, color: item.current ? 'var(--text-strong)' : 'var(--text-muted)' }}>{item.role}</span>
