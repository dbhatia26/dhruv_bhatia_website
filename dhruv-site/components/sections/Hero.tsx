...
function DataNetworkCanvas() {
  ...
  useEffect(() => {
    ...
    setInterval(() => {
      if (packets.length < 8) {
        const f = Math.floor(Math.random() * nodes.length), t = Math.floor(Math.random() * nodes.length)
    ...
    const draw = () => {
      ...
      for (let i = packets.length - 1; i >= 0; i--) {
        ...
        ctx.fillStyle = `rgba(0,212,255,${alpha})`; ctx.shadowColor = 'rgba(0,212,255,0.8)'; ctx.shadowBlur = 6; ctx.fill(); ctx.shadowBlur = 0
      }
      nodes.forEach((n) => {
        const glow = (Math.sin(n.pulse) + 1) * 0.5
        ctx.beginPath(); ctx.arc(n.x, n.y, n.r, 0, Math.PI * 2)
        ctx.fillStyle = `rgba(0,212,255,${0.2 + glow * 0.4})`; ctx.shadowColor = 'rgba(0,212,255,0.5)'; ctx.shadowBlur = 4 + glow * 6; ctx.fill(); ctx.shadowBlur = 0
      })
  ...
  return (
    <section className="relative min-h-screen flex items-center justify-center overflow-hidden grid-bg">
      ...
      <div className="relative z-10 max-w-5xl mx-auto px-6 text-center">
        ...
        <p style={{ fontFamily: 'DM Mono, monospace' }} className="text-sm tracking-[0.25em] uppercase text-white/30 mb-8">Lead Business Analyst · Data Operations</p>
        <p className="text-xl sm:text-2xl text-white/60 max-w-2xl mx-auto leading-relaxed mb-12 font-light">
          Building AI-powered analytics systems that help organizations <span className="text-white/90">make better decisions.</span>
        </p>
        ...
        <div className="flex flex-wrap items-center justify-center gap-4">
          <a href="#work" className="px-6 py-3 bg-cyan-400/10 border border-cyan-400/30 text-cyan-400 text-sm font-mono tracking-wider uppercase hover:bg-cyan-400/15 hover:border-cyan-400/60 transition-all rounded">View Work</a>
          <a href="#genie" className="px-6 py-3 bg-white/[0.03] border border-white/[0.08] text-white/60 text-sm font-mono tracking-wider uppercase hover:bg-white/[0.06] hover:text-white/80 transition-all rounded">Try Genie Demo ↓</a>
        </div>
