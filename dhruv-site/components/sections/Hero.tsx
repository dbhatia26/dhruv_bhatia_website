  ...
  return (
    <section id="hero" style={{ position: 'relative', minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden', paddingTop: '60px' }} className="grid-bg">
      <style>{`
        ...
        .hero-metrics { display: flex; flex-wrap: wrap; gap: 36px; justify-content: center; margin-bottom: 44px; }
      ...
      <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, height: '200px', pointerEvents: 'none', background: 'linear-gradient(to top, var(--bg), transparent)' }} />
      <div style={{ position: 'relative', zIndex: 10, maxWidth: '1000px', margin: '0 auto', padding: '40px 20px', textAlign: 'center', width: '100%' }}>
        ...
        {/* Status pill */}
        <div style={{ display: 'inline-flex', alignItems: 'center', gap: '8px', padding: '6px 14px', borderRadius: '100px', border: '1px solid var(--border)', background: 'var(--surface)', marginBottom: '28px' }}>
        ...
        {/* Name */}
        <h1 style={{ fontFamily: 'Syne, sans-serif', fontSize: 'clamp(2.4rem,8vw,5rem)', fontWeight: 800, lineHeight: 1.05, letterSpacing: '-0.02em', marginBottom: '16px' }}>
        ...
        {/* Statement */}
        <p style={{ fontSize: 'clamp(1rem,2.5vw,1.25rem)', color: 'var(--text-muted)', maxWidth: '520px', margin: '0 auto 36px', lineHeight: 1.65, fontWeight: 300, padding: '0 8px' }}>
        ...
        <div className="hero-metrics">
          {[{ value: '100+', label: 'AI queries / week' }, { value: '20+', label: 'Dashboards replaced' }, { value: '3×', label: 'Team multiplier' }].map((m) => (
            <div key={m.label} style={{ textAlign: 'center' }}>
              <div className="gradient-text-static" style={{ fontFamily: 'Syne, sans-serif', fontSize: 'clamp(1.6rem,4vw,2rem)', fontWeight: 700, lineHeight: 1, marginBottom: '4px' }}>{m.value}</div>
        ...
        <div className="hero-btns">
          <button onClick={() => scrollToSection('work')} style={{ padding: '12px 28px', background: 'rgba(0,212,255,0.09)', border: '1px solid rgba(0,212,255,0.32)', color: 'var(--cyan)', fontFamily: 'DM Mono, monospace', fontSize: '11px', letterSpacing: '0.14em', textTransform: 'uppercase', borderRadius: '5px', cursor: 'pointer', transition: 'all 0.2s' }}>
            View Work
          </button>
          <button onClick={() => scrollToSection('genie')} style={{ padding: '12px 28px', background: 'var(--surface)', border: '1px solid var(--border)', color: 'var(--text-muted)', fontFamily: 'DM Mono, monospace', fontSize: '11px', letterSpacing: '0.14em', textTransform: 'uppercase', borderRadius: '5px', cursor: 'pointer', transition: 'all 0.2s' }}>
            Try Genie Demo ↓
          </button>
        </div>
